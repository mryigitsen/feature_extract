...
import * as wav1 from "wav"
export function WavManager(open_callback, data_callback, end_callback) {
    var source = new Buffer.alloc(1);
    var fs = fs1;
    var wav = wav1;
    ...
    this.open = async function (path) {
        var file = fs.createReadStream(path);

        var source = new Buffer.alloc(1); //empty the current source if needed
        ...
        reader.on("end", async function () {
            ...
            if (_ecb) await _ecb(output, file_name);
        });
